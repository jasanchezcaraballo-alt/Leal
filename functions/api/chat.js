...
const EMPUJE_CONSEJO =
  "\n\n(Nota interna: ya conoces bastante la situación. Además de escuchar, en este mensaje ofrécele tu opinión sincera o un consejo concreto, " +
...
const PROMPT_RESUMEN =
  ...
  "Resume en 3 a 5 frases cortas, en tercera persona, los hechos importantes que contó la persona: qué le pasa, personas que menciona (con sus nombres si los dijo), " +
  "decisiones que ha tomado y cómo se sentía. Sin interpretar, sin diagnosticar y sin consejos. " +
  "Si habló de ideas de hacerse daño, indícalo en una frase. Responde solo con el resumen.";
...
export async function onRequestPost({ request, env }) {
  const { system, messages, mode } = await request.json();
  ...
  if (idxUltimo >= 0) {
    ...
    if (!cuidado) {
      ...
      else if (n >= 4 && (n - 4) % 3 === 0) extra = EMPUJE_CONSEJO;
    }
